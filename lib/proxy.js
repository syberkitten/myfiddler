...
proxy.on('error', function (err, req, res) {
    ...
    res.end('Something went wrong. proxyModule unable to route request');
});
...
function start_static_server(localPort) {
    ...
    log('starting static server, static_serve_url:',static_serve_url );
...
var proxyMiddler = function(req,res,next){
    ...
    log('--proxy reqUrl:',reqUrl.cyan);
    log('--proxy headers:',headers);
...
function start_proxy_server(port) {
    ...
    server = http.createServer(proxyApp).listen( port, function(){
        ...
        log('HTTP PROXY SERVER'.red + ' LISTENING ON PORT: '.green.bold + port.toString().yellow);
